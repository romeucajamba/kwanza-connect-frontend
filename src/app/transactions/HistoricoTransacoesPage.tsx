                    ...
                    return (
                      ...
                      >
                        ...
                        <td className="p-4 text-center">
                          <span className={`inline-flex items-center px-3 py-1 rounded-md text-[8px] font-bold uppercase tracking-widest ${tx.status === 'completed' ? 'bg-emerald-500/10 text-emerald-500' :
        ...
        <div className="md:hidden divide-y divide-slate-50 dark:divide-white/5">
          {paginatedTransactions && paginatedTransactions.length > 0 ? (
            paginatedTransactions.map((tx: any) => {
              ...
              const counterparty = isSeller ? tx.buyer : tx.seller;
              return (
                <div key={tx.id} onClick={() => setSelectedTx(tx)} className="p-4 flex items-center justify-between hover:bg-slate-50 dark:hover:bg-white/5 transition-colors active:scale-[0.98] cursor-pointer">
                  ...
                  <div className="flex flex-col items-end gap-2 flex-shrink-0">
                    ...
                    {tx.status === 'completed' && !tx.has_review && (
                      <button
                        onClick={(e) => { e.stopPropagation(); handleReview(e, tx.id); }}
                        className="h-6 px-2 bg-amber-500/10 text-amber-500 rounded text-[7px] font-black uppercase tracking-widest hover:bg-amber-500 hover:text-white transition-all flex items-center gap-1 mt-1"
                      >
              ...
              <div className="p-8 space-y-6">
                <div className="grid grid-cols-2 gap-8">
                  <div className="space-y-4">
                    ...
                    <div>
                      ...
                      <span className={`inline-flex px-2 py-0.5 rounded text-[8px] font-black uppercase tracking-widest ${selectedTx.status === 'completed' ? 'bg-emerald-500/10 text-emerald-500' : 'bg-amber-500/10 text-amber-500'
                        }`}>
                ...
                <div className="flex items-center gap-3 pt-4">
                  <button
                    ...
                    className="w-full h-11 bg-slate-100 dark:bg-white/5 text-slate-500 dark:text-slate-400 rounded-xl text-[10px] font-black uppercase tracking-widest hover:text-slate-700 transition-all font-sans"
                  >
                    Fechar
                  </button>
                </div>
