...
import { useSubmitReport } from '@services/auth.hooks';
...
const MensagensPage: React.FC = () => {
  ...
  // We derive selectedRoomId from URL to avert cascading renders caused by syncing manually
  ...
  if (selectedRoom?.offer) {
    offerId = typeof selectedRoom.offer === 'string'
      ? selectedRoom.offer
  ...
  const handleReportUser = () => {
    if (!otherUser || !reportReason.trim() || !selectedRoomId) return;
    submitReport({
      reported_to_id: otherUser.id,
  ...
  if (selectedRoom) {
    const member = selectedRoom.members?.find((m) => m.user?.id !== user_id);
    otherUser = member?.user || null;
  }
      ...
      <aside className={`flex-col bg-white dark:bg-[#192633] rounded-xl border border-slate-100 dark:border-white/5 shadow-md overflow-hidden transition-all duration-300 flex w-full md:w-80 lg:w-96 ${selectedRoomId ? 'hidden md:flex' : 'flex'}`}>
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar">
          {isLoadingRooms ? (
            ...
            // Só é considerada não lida se o servidor disser que há, e se não foi eu a enviar
            const hasUnread = room.unread_count > 0 && !isMeLast;
            ...
            <header className="p-4 border-b border-slate-50 dark:border-white/5 flex items-center justify-between bg-white dark:bg-[#192633] z-10 shadow-sm">
              ...
              <div className="flex items-center gap-2">
                ...
                <div className="flex items-center gap-1.5 ml-1 border-l border-slate-100 dark:border-white/5 pl-3">
                  <button onClick={() => setShowReportModal(true)} className="size-9 rounded-xl text-slate-400 hover:text-rose-500 hover:bg-rose-50 dark:hover:bg-rose-500/10 transition-all items-center justify-center flex" title="Denunciar Utilizador">
          ...
          <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-sm">
            ...
            >
              <div className="p-8">
                ...
                <p className="text-[11px] font-bold text-center text-slate-400 mt-3 uppercase tracking-widest leading-relaxed">
                  Descreva detalhadamente o motivo da denúncia. Nossa equipa analisará o histórico deste chat.
                </p>
                ...
                <div className="mt-6">
                  <textarea
                    ...
                    placeholder="Ex: Utilizador tenta redirecionar para fora da plataforma..."
                    className="w-full bg-slate-50 dark:bg-[#111922] border-none rounded-xl p-4 text-xs text-slate-900 dark:text-white focus:ring-2 focus:ring-rose-500/50 outline-none resize-none placeholder:text-slate-400"
                  />
                </div>
                ...
                <div className="mt-8 flex gap-3">
                  <button
                    ...
                    className="flex-1 h-12 rounded-2xl bg-slate-50 dark:bg-white/5 text-slate-400 text-[10px] font-black uppercase tracking-widest hover:text-slate-600 transition-all font-bold"
                  >
                    Cancelar
                  </button>
                  <button
                    ...
                    className="flex-[1.5] h-12 bg-rose-500 text-white rounded-2xl text-[10px] font-black uppercase tracking-widest shadow-lg shadow-rose-500/20 hover:bg-rose-600 transition-all flex items-center justify-center gap-2 disabled:opacity-50"
                  >
                    {isReporting ? (
